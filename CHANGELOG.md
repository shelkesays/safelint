...
- **SAFE110 now respects closure-local shadowing (Rust).** Descending into closures meant a closure's *own* `let mut failed` counted as a usage of a same-named outer binding, suppressing a valid finding. Shadowing is tracked positionally, because it begins at the shadowing `let`: a mutation *before* it is still the outer binding's, so treating the whole closure as shadowed would reintroduce the E0594 advice the fix above removes. Closure parameters shadow the entire body. A shadowing `let`'s own initializer is excluded from the shadow, because it is evaluated in the enclosing scope: `let failed = &mut failed;` mutably borrows the OUTER binding, and cutting the scan off at the `let`'s start hid that borrow. Typed closure parameters (`|mut failed: bool|`) shadow as well; they are wrapped in a `parameter` node rather than being bare identifiers. Shapes that cannot be read unambiguously - a tuple-pattern binding, a `let` nested in an inner block - are not claimed as shadows, keeping the rule quiet rather than risking a wrong suggestion. No change in findings across 2113 real Rust files (Ruff 310, ripgrep 42, both unchanged), which is the evidence that it adds no false positives; the gap it closes is real but rare in practice. Raised in review of PR #184.
...
- **Real-world validation harness** (`scripts/validate_real_world.py`). Runs one open-source project for one language against a given safelint binary - twice, with every rule enabled and then with stock defaults - and writes a committed summary to `plan/real-world-results/` with per-rule counts, the project's commit SHA, the safelint version, wall time and peak memory. It asks the binary under test for its rule registry (`list-rules --format json`) so config generation cannot drift from the version being validated, passes config via `--config` so a `safelint.toml` inside the cloned project cannot silently override it, filters results to the language's extensions, and excludes `vendor/` / `target/` / `third_party/` on top of the built-in exclusions. Dev tooling only - nothing in the wheel changes. The programme it serves, and the fifteen rule defects its first pass found (#153-#166), are in `plan/real-world-validation.md`; the first committed result is the published 2.14.0 against Spring PetClinic.
  ...
  Three harness defects found in review of PR #184 and fixed before the release: `--preset` plus `--pydantic` emitted two `[python]` tables, which TOML forbids, so the generated config was unparseable and every Django / Flask / FastAPI row of the matrix would have failed on it; a mistyped `--include` filtered away every finding while `files_checked` stayed positive from the rest of the project, so the harness wrote (and would have committed) a confident zero-findings report for a subtree it never validated, and an include selecting no source file of the target language is now an error; and naming a nonexistent `--safelint` binary raised a `FileNotFoundError` traceback instead of the documented `error:` line and exit 2, because version discovery ran ahead of the error guard. A binary that exits 0 with output that is not the expected `list-rules` JSON now takes that same path rather than raising `JSONDecodeError`, and the include-eligibility check counts files only, so a directory named like a source file cannot satisfy it.
...
- **SAFE801: write-sinks moved to their own `assignment_sinks` list (breaking change to the 2.14.0rc1 behaviour, not to any released version).** The first cut matched assignment targets against the flat `sinks_*` list. Outside JavaScript those hold FUNCTION names, so with shipped defaults `c.args = u` (Rust), `p.Query = q` (Go), `$this->query = $_GET[...]` (PHP), `obj.run = u` (Python), `this.load = u` (Java) and `s.system = argv[1]` (C) all reported as injections. Write-sinks now live in `assignment_sinks_<lang>` (bare `assignment_sinks` for Python), which ships `innerHTML` for JavaScript and empty everywhere else, so a name must be declared to be treated as a write-sink. `receiver_sinks` and `assignment_sinks` are bundled into one `SinkKinds` value object internally.
- **SAFE801: a computed subscript key is no longer matched as a property name.** `results[key] = tainted` and Java `arr[i] = tainted` matched the *spelling* of the index variable against the sink list, so any local called `run` / `load` fired. Only a quoted key (`o["danger"]`, which names the same property as `o.danger`) is matched now; Java array indices are not consulted at all.
- **SAFE801: restored taint through a nested f-string behind a non-propagating parent (Python).** The 2.14.0rc1 traversal prune made `eval(f"x{(f"y{u}")}z")` stop firing, because the inner string was then only reachable through a parent that is a taint dead-end. Replaced with a visited set in the worklist, which bounds the original 2^N blowup at its source without making anything unreachable - depth 150 stays under 0.01s.
- **SAFE801: `x, y = tainted, "k"` no longer marks `y` tainted.** An unpack is now paired positionally, matching what the Go tracker already did; mismatched arity or an unpack from a call still falls back to the conservative whole-RHS status.
- **SAFE801 `tainted_sink`: assignment-side sinks are now detected (JavaScript / TypeScript).** `innerHTML` has shipped in the default `sinks_javascript` since the rule existed, but hits were only ever recorded at a **call** - so the idiomatic `element.innerHTML = userInput`, the canonical DOM-XSS shape, was never reported, and the default could fire only on an `innerHTML(...)` call that does not occur in real code. Writing to the property IS the injection, so an assignment whose target names a configured sink is now checked like a call: `el.innerHTML = tainted`, the `el["innerHTML"] = tainted` subscript spelling, and `el.innerHTML += tainted` all fire. This works for **any** name in the sinks list, so `outerHTML` / `srcdoc` can be opted in the same way, and it honours the same sanitiser and property-typed contract as the call path (`el.innerHTML = escape(u)` is clean; a sanitiser establishing a different property is not). `textContent` and other non-sink properties stay clean. Implemented in **all seven trackers** on each language's native member shape (`obj.attr` / `obj["attr"]` in Python, `field_access` / `array_access` in Java, field expressions in Rust, Go, C and C++, `$obj->prop` in PHP), so a project-specific setter can be declared as a sink in any language. Go pairs its sides positionally, matching its existing multi-assign handling. `innerHTML` is the only shipped default of this shape; every other language's defaults are call-style and unaffected. **Expect new findings** if you have `tainted_sink` enabled on a DOM codebase - they are real, and were being missed.
- **SAFE801 `tainted_sink`: a cleared safety property no longer survives an unknown call (all seven trackers).** A property established by a property-typed sanitiser was carried through an intervening unknown call, so `html_sink(html_unescape(escape(user_input)))` read **clean** - an unknown call may transform or *decode* its input and cannot be trusted to preserve a safety property. The worklist now carries a per-node property and drops it when descending into an unknown call's arguments, so any taint reaching them re-taints the result for every property. Raw taint propagation is unchanged, and a direct clear (`html_sink(escape(u))`) still clears. Applied to Python, JavaScript/TypeScript, Java, Rust, Go, PHP, C and C++, with a parametrised cross-language parity test so the semantic cannot drift in one tracker.
- **SAFE801: C++ reference declarations bound no taint.** `const std::string& r = tainted;` (and the `T&&` form) silently lost taint: tree-sitter-cpp gives `reference_declarator` no `declarator` field, so the declarator descent bottomed out and bound nothing, leaving `r` clean at every later sink.
- **Explicitly instantiated C++ template calls now resolve to their bare name.** `execute<int>(x)` resolved to *no* name at all and `db.query<Row>(x)` to the literal text `"query<Row>"`, so neither ever matched a sink configured as `execute` / `query`. This affected **every** name-filtered rule on C++, not just SAFE801.
- **SAFE801: Python tuple-unpacking assignment lost taint.** The right-hand side of `x, y = user_input, "k"` parses as `expression_list` (not `tuple`), which was in no taint-propagating set, so *both* targets read clean. Go already handled its own `expression_list` positionally and JS destructuring was already covered; this was Python-only.
- **SAFE801: exponential time *and* memory on nested f-strings / template literals.** Collecting interpolations walked the entire string subtree at every nesting level, so each level re-enumerated every deeper one: N nested f-strings cost 2^N node visits whenever the walk had to exhaust (an untainted expression). Measured at 0.08s / 0.33s / 1.33s for N = 16 / 18 / 20. Because the worklist keeps no visited set, the re-pushed nodes also accumulated in the stack, so the blowup was 2^N in **memory** as well - roughly a million live node references at N = 20, and deep enough nesting exhausted system memory and was OOM-killed rather than merely slow. Nested strings are now pruned from the inner walk and handed to the worklist instead, which bounds both: the N = 20 case drops to 0.0001s, and N = 64 (previously an OOM) completes in 0.0002s with flat memory. Same fix in the Python and JavaScript trackers.
- **SAFE801: taint lost through an untracked compound assignment target (Java).** A chained assignment ending in a compound step against a non-identifier target (`a = obj.f += tainted`) returned clean. Regression introduced by the 2.13.0 compound-chain change.
- **SAFE801: Python config lists were not validated.** `sinks` / `sanitizers` / `sources` skipped the string-list guard on the Python path only, so a mistyped scalar (`sinks = "eval"`) silently became a set of single characters. Python now gets the same validation as every other language without losing its built-in fallback lists.
- **A mistyped config value is now an error message, not a traceback.** `sinks = "eval"` (missing brackets) escaped the CLI as an uncaught `TypeError` and dumped a full Python traceback, which reads like a crash in safelint rather than a mistake in the user's config. It now prints one `safelint: error: sinks must be a list of strings, got str` line and exits 2. The new `ConfigValueError` subclasses `TypeError`, so any caller catching `TypeError` is unaffected, and a genuine `TypeError` from a bug in safelint still raises with its traceback intact.
- **`PropertyContract` was hashable in name only and only shallowly frozen.** Hashing any contract raised `TypeError` (the generated `__hash__` hashed a mapping), and `frozen=True` did not stop a caller mutating the dict it had passed in, under every tracker holding it. Both tables are now copied into read-only mappings and hashed by content.
- **SAFE801: dead work removed.** `PropertyContract.all_properties` returned the *union* of declared sanitiser and sink properties, so any property no sink required cost a full extra tree traversal per assignment; it now intersects. The Go tracker regained a short-circuit for a value already safe for nothing, and a dead `annotated_assignment` dispatch branch (no such tree-sitter-python node kind) was removed.

### Added

- **SAFE801: warnings for property declarations that cannot take effect.** The property tables fail *open*, so a declaration that does not line up with the rest of the config simply never fires - indistinguishable, from the outside, from the feature being broken. Four cases now surface on stderr as `safelint: warning:` (visible, never failing the run, emitted once per language per engine - `safelint check` builds one engine per target, so a multi-path run repeats them per target, which matches config being resolved per target): a name in **both** the flat `sanitizers` list and `sanitizer_properties` (the flat list is universal and checked first, so adoption is a silent no-op); properties declared but **no sink requiring any of them**; a `sink_properties` key naming **no configured sink** (typo guard, matching the other config layers); and a sanitiser declared with an **empty property list** (it establishes nothing, and if it is not also in the flat list it has silently stopped sanitising). A well-formed contract warns nothing.
...
- **SAFE801 `tainted_sink` no longer false-positives on a tainted receiver passed only constant arguments.** The 2.11.0 method-receiver taint step reported an injection for `conn.execute("SELECT 1")` when `conn` derived from user input, even though the query is a hard-coded constant and no attacker data reaches the sink. The receiver is now treated as the payload **only when the sink call has no arguments** (`tainted.execute()`); when the call takes arguments, the payload is the argument(s), so a tainted receiver with all-constant arguments does not report (a tainted argument still fires via the argument path). Applied consistently across all seven trackers - Python, JavaScript/TypeScript, Java, Rust, Go, PHP, and the C++ path in `dataflow_c.py`; C remains excluded (its receiver step is off by design). In Java this also stops a tainted receiver being double-reported alongside a tainted argument.
  - **Exception for receiver-payload sinks** via a new `receiver_sinks` config list (`receiver_sinks_<lang>`, bare `receiver_sinks` for Python). Some sinks carry their payload in the *receiver*, not an argument - `url.openConnection(proxy)` is SSRF even when `proxy` is clean. Sinks listed here fire on a tainted receiver **regardless of arguments**, so the no-arguments rule (which is correct for argument-consuming sinks like `execute`/`query`) does not miss them. Defaults ship Java's URL SSRF methods (`openConnection`, `openStream`); add your own project's receiver-payload sinks per language (they must also appear in the matching `sinks_<lang>` list). Regression tests (constant-argument false-positive, receiver-as-payload true-positive, and the receiver-sink-with-auxiliary-argument case) added across the trackers.
- **SAFE908 `csrf_protection_disabled` no longer false-positives on a `csrf_exempt` keyword-argument value.** `@register(handler=csrf_exempt)` passes `csrf_exempt` as a keyword-argument *value*; it does not apply the `@csrf_exempt` decorator. The guard previously excluded `csrf_exempt` only as a keyword-argument *name* (`@foo(csrf_exempt=True)`); it now also excludes an unrelated keyword-argument value whether bare (`handler=csrf_exempt`), dotted (`handler=mod.csrf_exempt`) or nested in a call (`handler=wrapper(csrf_exempt)`) - the enclosing keyword argument is detected structurally, so an intervening `attribute` or `call` node no longer defeats the check. The real decorator forms still fire: bare `@csrf_exempt`, called `@csrf_exempt()`, positional `@method_decorator(csrf_exempt)`, and the `@method_decorator(decorator=csrf_exempt)` keyword form (including a dotted `decorator=mod.csrf_exempt` value, and a module-level `method_decorator` import alias), which genuinely applies the decorator to a class-based view. The `decorator=` keyword is only treated as applying when its enclosing call is `method_decorator` (or an alias of it), so an unrelated `@register(decorator=csrf_exempt)` does not fire.

### Changed

- **CI: bump pinned GitHub Actions** - `astral-sh/setup-uv` 9.0.0 -> 10.1.0 and `anthropics/claude-code-action` 1.0.185 -> 1.0.226 (both SHA-pinned). No workflow behaviour change: setup-uv v10 only alters caching on `pull_request_target` / `workflow_run` / `release` events, none of which trigger the setup-uv-using workflows here.
...
- **Two new framework-preset security rules - SAFE908 `csrf_protection_disabled` and SAFE909 `hardcoded_secret`** (Python + PHP; SafeLint now ships **52 rules**). They complete the shared cross-framework 9xx band (SAFE905-909) begun in v2.9.0, and like the others are disabled by default and switched on by the framework presets that scope them.
  - **SAFE908 `csrf_protection_disabled`** (error) - CSRF protection explicitly turned off in code: Django `@csrf_exempt` (bare, called, or `method_decorator(csrf_exempt)`), Laravel a non-empty `$except` route allow-list on the `VerifyCsrfToken` middleware (an empty `$except = []` is clean). Enabled by the **Django** and **Laravel** presets (Flask's CSRF is extension-provided and FastAPI has none, so neither enables it).
  - **SAFE909 `hardcoded_secret`** (error) - a secret key assigned a string literal: Django `SECRET_KEY = "..."`, Flask `app.secret_key = "..."`, Laravel a hardcoded `'key' => 'base64:...'` `APP_KEY` config entry. Reading from the environment (`os.environ` / `env(...)`) is a call, not a literal, and is clean. Enabled by the **Django**, **Flask**, and **Laravel** presets.
  - Both are more false-positive-prone than SAFE905-907 (a same-named decorator / property, or a placeholder literal in an example or test-settings file), so they stay opt-in even under vanilla and are documented as such. Bundled AI-client skill files (all 14) and the Python / PHP language pages / addenda document them.

### Changed

- **`tainted_sink` (SAFE801) now follows taint through attribute / subscript / method-receiver chains in Python, JavaScript / TypeScript, and C++ - closing a cross-language parity gap.** Request-driven web code almost never passes a bare tainted parameter to a sink; the data arrives behind a projection of the request object (`request.GET["q"]`, `req.query.get("q")`, `$request->input('x')`). Java, Rust, Go, PHP, and C already propagated taint through these projections; the gaps were narrower: **Python** dropped it at the first attribute access (it had none of the three steps), while **JavaScript / TypeScript** and **C++** already handled attribute / subscript access and only lacked the method-call-on-a-tainted-receiver step. So a framework preset's injection sinks (Django `RawSQL` / `mark_safe`, Flask `render_template_string`, Laravel `whereRaw`, ...) were largely inert on idiomatic request-driven code. Now, uniformly across all nine languages: an **attribute / member access** propagates its receiver's taint (`request.data`), a **subscript / index** propagates its base's taint (`request.GET["q"]`), and a **method call on a tainted receiver** stays tainted even with no tainted positional arguments (`request.GET.get("q")`). The method-receiver step rides on the existing conservative `assume_taint_preserving = true` default (matching how Java / Rust / Go / PHP already behave), so `assume_taint_preserving = false` still turns it off, and the sanitizer check runs first so `escape(request.data)` still clears. This can surface new findings on existing code that has the opt-in `tainted_sink` rule enabled - which is the point: it is the difference between detecting only `RawSQL(request)` (rare) and detecting `RawSQL(request.GET["q"])` (idiomatic). No new config key. Relatedly, **a sink invoked as a method on a tainted receiver now fires even with no tainted arguments** (`tainted.execute()`), uniformly across Python / JS / TS / Go / Rust / PHP / C++ - previously only Java did this. When both an argument and the receiver are tainted, the call is reported once (the argument), not twice. In C specifically this receiver step is **off**: `req->handler()` is a function-pointer call whose result need not derive from the struct, so treating the struct as a taint input there would be a false positive (C++ member calls, which are true methods, keep it).
...
- **safelint now lints its own test tree (repo hygiene; the `safelint` package is unchanged).** `tests/` was previously excluded wholesale via `extend_exclude_paths`; it is now linted, and the CI self-lint step runs `safelint check src/ scripts/ tests/ --all-files --fail-on=warning` in one invocation (enabled by the new multi-path support). Two `safelint.toml` per-file-ignores keep it honest: `tests/fixtures/**` = `["*"]` (the fixtures are deliberately non-compliant - they exist to make rules fire in the e2e tests, so they can never satisfy their own rules) and `tests/**` relaxes the structure/style rules that test scaffolding legitimately trips (`function_length`, `nesting_depth`, `logging_on_error`). Every other rule - complexity, no-recursion, dataflow, etc. - now applies to test code.
- **Release automation (internal tooling; the `safelint` package is unchanged).** The `development`->`main` post-release sync now decides "is it safe to reset development?" by **content** (patch-id via `git cherry`) instead of raw SHA commit count. A squash- or rebase-merged release leaves `development`'s commits content-identical to `main` but with different SHAs; the old count-based check read those as "26 commits ahead" and refused every release (opening a tracking issue and requiring a manual reset), even though nothing was actually unmerged. The patch-id check reads that case as 0 in-flight commits and auto-resolves, while still refusing when genuinely unmerged content exists. Because `git cherry` is patch-id based and by construction skips merge commits, the guard also refuses when a merge commit exists in `main..development` (which `git cherry` cannot patch-compare), so a merge whose conflict resolution held content unique to `development` cannot be silently clobbered - development is linear under the repo's squash/rebase flow, so this is normally zero and never blocks a routine release. Applied to both `.github/workflows/sync-development.yml` and the local `bin/sync.sh` advisor.
...
- **Python and PHP framework presets - Django / Flask / FastAPI / Pydantic (Python) and Laravel (PHP).** Mirroring the existing Java Spring Boot and JavaScript runtime presets, a `[tool.safelint.python] framework = "django" | "flask" | "fastapi"` key (plus an orthogonal composable `pydantic = true`) and a `[tool.safelint.php] framework = "laravel"` key shift the rule *defaults* for the chosen framework. The parser and rule logic are unchanged; only the taint sink lists, nullable-method lists, and which structural rules are active shift. The default is `vanilla` for both, so existing projects with no `[python]` / `[php]` config see no behaviour change. Unknown framework names warn on stderr and fall back to `vanilla`; explicit per-rule TOML keys always win over the preset.
  ...
  - **Preset coverage is mostly taint-list extension (zero new codes):** the Django / Flask / FastAPI / Laravel presets add framework-specific sinks (e.g. `RawSQL` / `mark_safe`, `render_template_string`, `HTMLResponse`, `whereRaw` / `unprepared`) to SAFE801 `tainted_sink`, and `pydantic = true` additively adds `model_construct` / `construct` to the SAFE801 sinks. Django also treats `.first()` as nullable for SAFE803. Consistent with the Java Spring preset, these dataflow additions extend the *lists* but leave the multi-language dataflow rules (SAFE801/802/803) opt-in - enable `tainted_sink` explicitly to use them (so a Python framework selection never turns dataflow on for Go / JS / Rust files in a polyglot repo). Like every language's intra-procedural tracker, SAFE801 follows direct taint flows, not `request.<attr>` attribute chains.
