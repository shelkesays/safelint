...
- **CI: bump pinned GitHub Actions** - `astral-sh/setup-uv` 9.0.0 -> 10.0.1 and `anthropics/claude-code-action` 1.0.185 -> 1.0.213 (both SHA-pinned). No workflow behaviour change: setup-uv v10 only alters caching on `pull_request_target` / `workflow_run` / `release` events, none of which trigger the setup-uv-using workflows here.
