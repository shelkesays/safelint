...
- **SAFE801: quadratic-to-exponential traversal on nested f-strings / template literals.** Collecting interpolations walked the entire string subtree at every nesting level, so each level re-enumerated every deeper one: N nested f-strings cost 2^N node visits whenever the walk had to exhaust (an untainted expression). Measured at 0.08s / 0.33s / 1.33s for N = 16 / 18 / 20. Nested strings are now pruned from the inner walk and handed to the worklist instead - linear, and the N=20 case drops to 0.0001s. Same fix in the Python and JavaScript trackers.
