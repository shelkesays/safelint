...
- **SAFE908 `csrf_protection_disabled` no longer false-positives on a `csrf_exempt` keyword-argument value.** `@register(handler=csrf_exempt)` passes `csrf_exempt` as a keyword-argument *value*; it does not apply the `@csrf_exempt` decorator. The guard previously excluded `csrf_exempt` only as a keyword-argument *name* (`@foo(csrf_exempt=True)`); it now excludes a keyword argument on either side (name or value), while the real decorator forms - bare `@csrf_exempt`, called `@csrf_exempt()`, and positional `@method_decorator(csrf_exempt)` - still fire.
