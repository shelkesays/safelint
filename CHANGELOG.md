...
- **SAFE801 `tainted_sink`: assignment-side sinks are now detected (JavaScript / TypeScript).** `innerHTML` has shipped in the default `sinks_javascript` since the rule existed, but hits were only ever recorded at a **call** - so the idiomatic `element.innerHTML = userInput`, the canonical DOM-XSS shape, was never reported, and the default could fire only on an `innerHTML(...)` call that does not occur in real code. Writing to the property IS the injection, so an assignment whose target names a configured sink is now checked like a call: `el.innerHTML = tainted`, the `el["innerHTML"] = tainted` subscript spelling, and `el.innerHTML += tainted` all fire. This works for **any** name in the sinks list, so `outerHTML` / `srcdoc` can be opted in the same way, and it honours the same sanitiser and property-typed contract as the call path (`el.innerHTML = escape(u)` is clean; a sanitiser establishing a different property is not). `textContent` and other non-sink properties stay clean. **JavaScript / TypeScript only:** every other language recognises a sink solely when it is *called*, so configuring a property-style sink name there still has no effect on assignments (no shipped default in those languages is property-style, so no default is affected). **Expect new findings** if you have `tainted_sink` enabled on a DOM codebase - they are real, and were being missed.
