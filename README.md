...
| **C** | `.c`, `.h` | 21 rules apply: the 16 cross-language ports plus **5 new C-family rules** (shared with C++) (Holzmann's original language, clauses every other language adapts away): `nonlocal_jumps` (`goto` / `setjmp`, rule 1), `dynamic_allocation` (`malloc` family, rule 3), `complex_macro` + `conditional_compilation` (preprocessor, rule 8), `restricted_pointers` (rule 9). `nonlocal_jumps` is enabled (warning); the rest opt-in. `.h` lints as C. New in v2.7.0. |
