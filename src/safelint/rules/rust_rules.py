...
_RUST_ARITHMETIC_OPERATORS: frozenset[str] = frozenset({_rust.PLUS, _rust.MINUS, _rust.STAR})
...
def _node_has_test_marker_attribute(node: tree_sitter.Node) -> bool:
    ...
    while cursor is not None:
        if cursor.type != _rust.ATTRIBUTE_ITEM:
            break
...
def _is_in_test_context(node: tree_sitter.Node) -> bool:
    ...
    * any ancestor function / mod is ``#[cfg(test)]``-gated.
    ...
    while cursor is not None:
        if cursor.type in (_rust.FUNCTION_ITEM, _rust.MOD_ITEM) and _node_has_test_marker_attribute(cursor):
            return True
...
def _unwrap_to_scoped_identifier(func: tree_sitter.Node | None) -> tree_sitter.Node | None:
    ...
    if func is None:  # pragma: no cover - defensive: every call_expression has a function field
        return None
    if func.type == _rust.GENERIC_FUNCTION:
        ...
        if func is None:  # pragma: no cover - defensive: every generic_function has an inner function field
            return None
    return func if func.type == _rust.SCOPED_IDENTIFIER else None
...
class UndocumentedUnsafeRule(BaseRule):
    ...
    def check_file(self, filepath: str, tree: tree_sitter.Tree) -> list[Violation]:
        ...
        violations: list[Violation] = []
        for node in walk(tree.root_node):
            if node.type != _rust.UNSAFE_BLOCK:
                continue
    ...
    def _has_safety_comment(unsafe_block: tree_sitter.Node) -> bool:
        ...
        if parent is not None and parent.type == _rust.EXPRESSION_STATEMENT:
            anchor = parent
        prev = anchor.prev_sibling
        while prev is not None:
            if prev.type not in (_rust.LINE_COMMENT, _rust.BLOCK_COMMENT):
                break
...
def _is_err_pattern(pattern: tree_sitter.Node | None) -> bool:
    ...
    """
    if pattern is None or pattern.type != _rust.TUPLE_STRUCT_PATTERN:
        return False
    children = pattern.named_children
    if not children or children[0].type != _rust.IDENTIFIER:  # pragma: no cover - defensive: tuple_struct_pattern always has a leading identifier
        return False
...
def _match_arm_pattern_and_body(arm: tree_sitter.Node) -> tuple[tree_sitter.Node | None, tree_sitter.Node | None]:
    ...
    if len(children) < 2 or children[0].type != _rust.MATCH_PATTERN:  # pragma: no cover - defensive: every match_arm has match_pattern + body
        return None, None
...
def _if_let_err_pattern_and_body(if_expr: tree_sitter.Node) -> tuple[tree_sitter.Node | None, tree_sitter.Node | None]:
    ...
    """
    condition = next((c for c in if_expr.named_children if c.type == _rust.LET_CONDITION), None)
    ...
    if not _is_err_pattern(pattern):
        return None, None
    block = next((c for c in if_expr.named_children if c.type == _rust.BLOCK), None)
    return pattern, block


_NOOP_LEAF_TYPES: frozenset[str] = frozenset(
    {
        _rust.INTEGER_LITERAL,
        _rust.FLOAT_LITERAL,
        _rust.STRING_LITERAL,
        _rust.CHAR_LITERAL,
        _rust.BOOLEAN_LITERAL,
        _rust.UNIT_EXPRESSION,
    }
)
...
def _block_is_noop(block: tree_sitter.Node) -> bool:
    ...
    if only.type in _NOOP_LEAF_TYPES:
        return True
    if only.type != _rust.EXPRESSION_STATEMENT:  # pragma: no cover - rare: noop-leaf single-stmt blocks parse as direct leaf (tail form) or expression_statement (semicolon-terminated)
        return False
...
def _body_propagates_or_panics(body: tree_sitter.Node | None) -> bool:
    ...
    * Tail-position re-raise: the body's single tail expression is
    ...
    for node in walk(body, skip_types=_RUST_FUNCTION_TYPES_FOR_SKIP):
        if node.type == _rust.RETURN_EXPRESSION or _node_is_panic_like_macro(node):
            return True
...
def _body_tail_is_err_constructor(body: tree_sitter.Node) -> bool:
    ...
    if body.type == _rust.CALL_EXPRESSION and _is_err_constructor_call(body):
        return True
    if body.type != _rust.BLOCK:
        return False
    ...
    if not named:  # pragma: no cover - defensive: callers reach this only for non-empty bodies
        return False
    tail = named[-1]
    if tail.type == _rust.CALL_EXPRESSION:
    ...
    if tail.type == _rust.EXPRESSION_STATEMENT:
        inner = next((c for c in tail.named_children), None)
        return inner is not None and inner.type == _rust.CALL_EXPRESSION and _is_err_constructor_call(inner)
...
def _is_err_constructor_call(call_node: tree_sitter.Node) -> bool:
    ...
    func = call_node.child_by_field_name("function")
    if func is None or func.type != _rust.IDENTIFIER:
    ...
    * ``if let Err(_) = res { }`` - empty if-let-Err body.
    ...
    def _check_node(self, filepath: str, node: tree_sitter.Node) -> Violation | None:
        ...
        if node.type == _rust.MATCH_ARM:
            ...
            return None
        if node.type == _rust.IF_EXPRESSION:
...
class TruncatingAsCastRule(BaseRule):
    ...
    def check_file(self, filepath: str, tree: tree_sitter.Tree) -> list[Violation]:
        ...
        violations: list[Violation] = []
        for node in walk(tree.root_node):
            if node.type != _rust.TYPE_CAST_EXPRESSION:
                continue
    ...
    def _cast_target_primitive(cast_node: tree_sitter.Node) -> str | None:
        ...
        if len(named) < 2:  # pragma: no cover - defensive: type_cast_expression has source + type
            return None
        target = named[-1]
        return node_text(target) if target.type == _rust.PRIMITIVE_TYPE else None
...
def _let_mut_binding_name(let_decl: tree_sitter.Node) -> str | None:
    ...
    """
    if not any(c.type == _rust.MUTABLE_SPECIFIER for c in let_decl.named_children):
        return None
...
def _node_is_mut_use_of(name: str, node: tree_sitter.Node) -> bool:
    ...
    if node.type == _rust.FIELD_EXPRESSION:
...
def _assignment_left_is(name: str, node: tree_sitter.Node) -> bool:
    ...
    return left is not None and left.type == _rust.IDENTIFIER and node_text(left) == name


def _is_mut_reference_of(name: str, node: tree_sitter.Node) -> bool:
    """Return True if *node* is ``&mut <name>``."""
    if not any(c.type == _rust.MUTABLE_SPECIFIER for c in node.named_children):
        return False  # pragma: no cover - bare ``&x`` references aren't mut-needing usages
    inner = next((c for c in node.named_children if c.type == _rust.IDENTIFIER), None)
...
def _field_expression_value_is(name: str, node: tree_sitter.Node) -> bool:
    ...
    return value is not None and value.type == _rust.IDENTIFIER and node_text(value) == name
...
def _first_named_child_is(name: str, node: tree_sitter.Node) -> bool:
    ...
    return bool(children) and children[0].type == _rust.IDENTIFIER and node_text(children[0]) == name  # pragma: no cover - helper for the rarely-hit index_expression branch
    ...
    * ``name[i]`` (index expression - same reasoning).
    ...
    def _violations_in_body(self, filepath: str, body: tree_sitter.Node) -> list[Violation]:
        ...
        for let_decl in walk(body, skip_types=_RUST_FUNCTION_TYPES_FOR_SKIP):
            if let_decl.type != _rust.LET_DECLARATION:
                continue
...
def _integer_param_names(func_node: tree_sitter.Node) -> set[str]:
    ...
    for child in params_node.named_children:
        ...
        if type_node is None or type_node.type != _rust.PRIMITIVE_TYPE:
            continue
        if node_text(type_node) not in _RUST_INTEGER_PRIMITIVE_TYPES:
            continue
        pattern = child.child_by_field_name("pattern")
        if pattern is None or pattern.type != _rust.IDENTIFIER:
            continue
...
class UncheckedArithmeticOnInputRule(BaseRule):
    ...
    * At least one operand is an ``identifier`` whose text matches
    ...
    def check_file(self, filepath: str, tree: tree_sitter.Tree) -> list[Violation]:
        ...
        violations: list[Violation] = []
        for func_node in walk(tree.root_node):
            if func_node.type != _rust.FUNCTION_ITEM:
                continue
    ...
    def _violations_in_body(self, filepath: str, body: tree_sitter.Node, int_params: set[str]) -> list[Violation]:
        ...
        out: list[Violation] = []
        for node in walk(body, skip_types=_RUST_FUNCTION_TYPES_FOR_SKIP):
            if node.type != _rust.BINARY_EXPRESSION:
                continue
    ...
    def _operand_matching_param(binary: tree_sitter.Node, int_params: set[str]) -> str | None:
        ...
        for field in ("left", "right"):
            ...
            if operand is None or operand.type != _rust.IDENTIFIER:
...
def _op_method_suffix(op: str | None) -> str:
    ...
    return {_rust.PLUS: "add", _rust.MINUS: "sub", _rust.STAR: "mul"}.get(op or "", "op")
...
def _static_item_is_mutable_specifier(static_node: tree_sitter.Node) -> bool:
    ...
    """
    return any(child.type == _rust.MUTABLE_SPECIFIER for child in static_node.named_children)
...
class InteriorMutableStaticRule(BaseRule):
    ...
    * A ``lazy_static! { ... }`` macro invocation - the macro's whole
    ...
    def _violation_for(self, filepath: str, node: tree_sitter.Node, names: frozenset[str]) -> Violation | None:
        ...
        if node.type == _rust.MACRO_INVOCATION and _rust_macro_name(node.child_by_field_name("macro")) == "lazy_static":
            return self._make_violation_for_node(
                filepath,
                node,
        ...
        if node.type != _rust.STATIC_ITEM or _static_item_is_mutable_specifier(node):
            return None
