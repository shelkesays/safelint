...
| SAFE603 | blanket_suppression | Flags blanket golangci-lint / Staticcheck directives (Holzmann rule 10): golangci-lint's bare `//nolint` (all linters) and Staticcheck's bare `//lint:ignore` (no checks). Scoped forms (`//nolint:errcheck`, `//lint:ignore SA1000 reason`) are clean. Both are machine-readable directives recognised only with no space after `//`, so a prose `// nolint here` comment is not flagged. Disabled by default. |
...
| SAFE801 | tainted_sink | Vanilla sinks (`sinks_go`): `Command` / `CommandContext` (os/exec), `Query` / `QueryRow` / `Exec` (database/sql raw SQL), `Open` (intended as `plugin.Open`). **Matching is name-only** - there is no type information, so `Open` fires on ANY `.Open(...)` call with a tainted argument, `os.Open` included. Drop it from `sinks_go` if that is too noisy for your codebase. Vanilla sources (`sources_go`): `Getenv` (os), `FormValue` / `PostFormValue` / `FormFile` (net/http request). Sanitizers (`sanitizers_go`): narrow generic set (`sanitize` / `validate` / `escape` / `quote`). Collision rule: a name that is both a sink and a source resolves as the sink (`Query` is kept as a sink, dropped from sources). `os.Args` and `Header.Get` are omitted from default sources (non-call / collision-prone) - add via config if needed. **Property-typed sanitisers (2.13.0):** the flat `sanitizers_go` list is universal - a name there clears *every* sink. To recognise a context-specific encoder precisely, declare what it establishes in `sanitizer_properties_go` (name -> list of properties) and what each sink requires in `sink_properties_go` (sink -> one property); it then clears only the sinks whose required property it establishes. Keep such a name OUT of the flat list, which is checked first and would otherwise clear everything. Opt-in - both tables ship empty, so existing config is unaffected. |
