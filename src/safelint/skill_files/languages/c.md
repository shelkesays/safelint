...
C is Holzmann's original "Power of Ten" target language, so several clauses every other language *adapts away* apply literally here. C ports the 16 cross-language rules and adds **five new C-only rules** (the "homecoming") that express those clauses directly.
...
21 rules apply to C: the 16 cross-language ports plus the 5 new C-only rules (SAFE106 / SAFE310-313). Comments use `//` (line-directive form): `// nosafe`, `// nosafe: SAFE106`, `// safelint: ignore`.
...
| SAFE106 | nonlocal_jumps | *C-only (rule 1, literal).* Every `goto` and every `setjmp` / `longjmp` / `sigsetjmp` / `siglongjmp` call. **Enabled at warning severity** (idiomatic `goto err` cleanup surfaces without blocking `--fail-on=error`); annotate sanctioned cleanups with `// nosafe: SAFE106`. Configurable via `nonlocal_jump_calls_c`. |
...
| SAFE310 | dynamic_allocation | *C-only (rule 3, literal).* Calls to the `malloc` family. Default `allocation_calls_c`: `malloc` / `calloc` / `realloc` / `aligned_alloc` / `free` / `strdup`. Disabled by default (embedded / safety-critical opt-in). |
| SAFE311 | complex_macro | *C-only (rule 8).* Function-like macros using `##` token paste or `__VA_ARGS__`, and object-like macros whose replacement is not a balanced syntactic unit. Disabled by default. |
| SAFE312 | conditional_compilation | *C-only (rule 8).* Every `#if` / `#ifdef` / `#ifndef` beyond the `#ifndef X` + `#define X` include-guard idiom - each doubles the build configurations to test. Disabled by default. |
| SAFE313 | restricted_pointers | *C-only (rule 9, literal).* Declarators with more than one pointer level (`int **p`) and function-pointer declarators (`void (*fp)(int)`). Disabled by default. Note: `char **argv` fires too (it is a two-level pointer) - that is intentional for the strict rule. |
