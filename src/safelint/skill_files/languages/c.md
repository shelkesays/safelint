...
For pre-commit integration, the published hook routes `.c` / `.h` files via the `c` filetype tag in `types_or`:
...
| SAFE106 | nonlocal_jumps | *C / C++ (rule 1, literal).* Every `goto` and every `setjmp` / `longjmp` / `sigsetjmp` / `siglongjmp` call. **Enabled at warning severity** (idiomatic `goto err` cleanup surfaces without blocking `--fail-on=error`); annotate sanctioned cleanups with `// nosafe: SAFE106`. Configurable via `nonlocal_jump_calls_c`. |
...
| SAFE310 | dynamic_allocation | *C / C++ (rule 3, literal).* Calls to the `malloc` family. Default `allocation_calls_c`: `malloc` / `calloc` / `realloc` / `aligned_alloc` / `free` / `strdup`. Disabled by default (embedded / safety-critical opt-in). |
