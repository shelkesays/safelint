...
| SAFE801 | tainted_sink | Sources: the superglobals `$_GET` / `$_POST` / `$_REQUEST` / `$_COOKIE` / `$_SERVER` / `$_FILES` / `$_ENV`. Sinks: `eval` / `exec` / `system` / `shell_exec` / `passthru` / `proc_open`, `->query(...)` (raw SQL), `unserialize`, `include` / `require` (and the `_once` forms). Sanitizers: `intval` / `escapeshellarg` / `escapeshellcmd` / `htmlspecialchars` / `preg_quote` and friends, which clear taint. |
