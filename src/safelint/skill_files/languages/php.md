...
| SAFE801 | tainted_sink | Sources: the superglobals `$_GET` / `$_POST` / `$_REQUEST` / `$_COOKIE` / `$_SERVER` / `$_FILES` / `$_ENV`. Sinks: `eval` / `exec` / `system` / `shell_exec` / `passthru` / `popen` / `proc_open` (command), `unserialize` (object-injection), `query` / `mysqli_query` / `pg_query` (raw SQL), plus dynamic `include` / `require` (and the `_once` forms, matched as expressions rather than via `sinks_php`). Default sanitizers (clear taint): `intval` / `floatval` / `escapeshellarg` / `escapeshellcmd` plus the generic `sanitize` / `validate` / `escape` / `quote`. `htmlspecialchars` is deliberately NOT a default sanitizer (it is HTML-context-only and does not neutralise SQL / shell metacharacters). Since 2.13.0 the right way to recognise it is the property-typed contract below - `sanitizer_properties_php` - not the flat `sanitizers_php` list, which would also clear your SQL and shell sinks. **Property-typed sanitisers (2.13.0):** the flat `sanitizers_php` list is universal - a name there clears *every* sink. To recognise a context-specific encoder precisely, declare what it establishes in `sanitizer_properties_php` (name -> list of properties) and what each sink requires in `sink_properties_php` (sink -> one property); it then clears only the sinks whose required property it establishes. Keep such a name OUT of the flat list, which is checked first and would otherwise clear everything. Opt-in - both tables ship empty, so existing config is unaffected. |
