...
| SAFE105 | no_recursion | Flags a named function / method that calls itself directly, bare (`fact(n-1)`) or `this`-qualified. `other.walk(...)` does not fire. Anonymous arrow / function expressions are a documented blind spot. Enabled by default at warning severity. |
...
| SAFE309 | dynamic_code_execution | Structural detection of `eval` / `new Function(...)` / `Function(...)` / `execScript` (bare-identifier callee; `obj.eval()` does not fire). Holzmann rule 8; complements SAFE801. Disabled by default. |
...
| SAFE603 | blanket_suppression | Flags rule-less `eslint-disable` / `eslint-disable-line` / `eslint-disable-next-line`, plus `@ts-nocheck` / `@ts-ignore` (Holzmann rule 10). Rule-listed disables and `@ts-expect-error` are clean. Disabled by default. |
...
| SAFE801 | tainted_sink | Intra-procedural taint analysis. Function parameters seed the tainted set (including destructured names: `function f({userInput})` taints `userInput`; `function f([first, ...rest])` taints both). Taint propagates through `const` / `let` / `var` declarations, `assignment_expression`, `augmented_assignment_expression` (`+=`), template strings (`\`prefix ${tainted}\``), array / object literals, spread, and member / subscript access on tainted receivers. JS default sinks: `eval`, `Function` (constructor), `execScript`, `exec`, `execSync`, `execFile`, `execFileSync`, `spawn`, `spawnSync`, `setTimeout`, `setInterval`. `innerHTML` is NOT in this list - it is a write-sink, declared separately in `assignment_sinks_javascript`. **Assignment-side sinks (2.14.0):** a sink name written to rather than called - `element.innerHTML = tainted`, the `el["innerHTML"] = tainted` spelling, and `+=` - is reported, because writing the property IS the injection. Any name in `sinks_javascript` works this way, so `outerHTML` / `srcdoc` can be added the same way; every other language supports the same on its own member shape. `textContent` stays clean because it is not in `assignment_sinks_javascript`, not because it is special - adding it there would make `el.textContent = tainted` fire. JS default sanitizers: `escape`, `sanitize`, `encodeURIComponent`, `encodeURI`, `DOMPurify`, `validate`, `clean`. JS default sources: `prompt`, `readline`, `stdin`, `input`. Per-language config keys: `sinks_javascript` / `sanitizers_javascript` / `sources_javascript`. **Property-typed sanitisers (2.13.0):** the flat `sanitizers_javascript` list is universal - a name there clears *every* sink. To recognise a context-specific encoder precisely, declare what it establishes in `sanitizer_properties_javascript` (name -> list of properties) and what each sink requires in `sink_properties_javascript` (sink -> one property); it then clears only the sinks whose required property it establishes. Keep such a name OUT of the flat list, which is checked first and would otherwise clear everything. **This bites on JavaScript specifically:** seven names ship IN the default `sanitizers_javascript` - `escape`, `sanitize`, `encodeURIComponent`, `encodeURI`, `DOMPurify`, `validate`, `clean` - so declaring properties for ANY of them changes nothing until you also override `sanitizers_javascript` to drop that name. Until then `eval(encodeURI(userInput))` stays unreported. safelint warns at runtime when a name sits in both places, including when it comes from these shipped defaults rather than your own list. Opt-in - both tables ship empty, so existing config is unaffected. |
