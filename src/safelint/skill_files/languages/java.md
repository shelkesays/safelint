...
| SAFE105 | no_recursion | Flags a `method_declaration` that calls itself directly, bare (`fact(n-1)`) or `this`-qualified (`this.fact(n-1)`). `other.fact(...)` does not fire. Direct self-recursion only (mutual recursion out of scope). Enabled by default at warning severity. |
...
| SAFE302 | global_mutation | *(added in 2.4.0)* Fires on non-final `static` field declarations - Java's shared-mutable-state shape (Holzmann rule 6). Declaration-site detection. `static final` is clean (interior mutability of a `final` referent is a documented exclusion). Instance fields, locals, and interface fields (implicitly `public static final`) never fire. Enabled by default. |
| SAFE309 | dynamic_code_execution | Structural detection of reflection (Holzmann rule 8): `Class.forName` / `Method.invoke` / `eval` (JSR-223) / `defineClass` / `loadClass`, matched by method name. Complements SAFE801 (taint-gated). Disabled by default. |
...
| SAFE603 | blanket_suppression | Flags `@SuppressWarnings("all")` and `@SuppressWarnings({..., "all"})` (Holzmann rule 10). Scoped suppressions like `@SuppressWarnings("unchecked")` are clean. Disabled by default. |
...
These rules are NOT registered for Java because the Python / JS-family semantics don't translate cleanly:
...
| SAFE301 | global_state | Python / PHP (`global` keyword); not applicable to Java, which has no statement-level "this references module state" annotation. (Java shared-mutable-state is instead covered by SAFE302's non-final-static-field check, see the table above.) |
