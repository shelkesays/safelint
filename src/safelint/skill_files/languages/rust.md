...
Rust is in scope for the cross-language rules (SAFE101-105, SAFE303-304, SAFE501, SAFE601, SAFE603, SAFE701-702, SAFE801-803) plus the Rust-specific rules (SAFE110, SAFE112, SAFE204-208, SAFE306, SAFE307, SAFE308, SAFE602). New in 2.4.0: SAFE105 `no_recursion` (enabled by default), SAFE307 `interior_mutable_static` and SAFE603 `blanket_suppression` (disabled by default). The rules deliberately skipped for Rust (SAFE201, SAFE202, SAFE203, SAFE301, SAFE302, SAFE309, SAFE401) are listed in the "Deliberately skipped" section below.
...
| SAFE105 | no_recursion | Flags a `function_item` that calls itself directly, bare (`fact(n-1)`) or `self`-qualified (`self.walk(...)`). `other.walk(...)` does not fire. Direct self-recursion only (mutual recursion out of scope). Enabled by default at warning severity. |
...
| SAFE307 | interior_mutable_static | *Rust-only.* Fires on a `static` whose type provides safe interior mutability (`Mutex` / `RwLock` / `OnceLock` / `OnceCell` / `Lazy*` / `Atomic*` / `RefCell` / `Cell`) and on `lazy_static! { ... }` blocks - the safe global mutable state SAFE602's unsafe gate never sees (Holzmann rule 6). `const` and `static mut` are not flagged. Configurable via `interior_mutable_types_rust`. Disabled by default. |
...
| SAFE603 | blanket_suppression | Flags blanket allows (Holzmann rule 10): `#[allow(clippy::all)]`, `#[allow(warnings)]`, and inner `#![...]` forms. Scoped allows (`#[allow(dead_code)]`, `#[allow(clippy::too_many_arguments)]`) are clean. Disabled by default. |
...
| SAFE301 | global_state | Rust has no `global` keyword. `static mut FOO` is the closest analogue but requires `unsafe { }` to access, so it's already audit-gated via **SAFE602 `undocumented_unsafe`**. Safe interior-mutable statics are covered by **SAFE307 `interior_mutable_static`**. |
| SAFE302 | global_mutation | Same as SAFE301 - `static mut` writes are unsafe-gated (SAFE602), and safe interior-mutable statics are covered by **SAFE307 `interior_mutable_static`**. |
| SAFE309 | dynamic_code_execution | Rust's rule-8 analogue is the macro system, whose bodies parse as opaque token trees safelint cannot decode. `panic`-family macros already have SAFE204. No general macro-as-codegen rule is offered. |
