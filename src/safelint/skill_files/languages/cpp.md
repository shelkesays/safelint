...
For pre-commit integration you normally only need to add the grammar extra via `additional_dependencies`; the published hook's `types_or` already includes the `c++` filetype tag (alongside every other supported language), so C++ files are picked up without a `types_or` line unless you have customised `types_or` yourself. If you do override `types_or`, note that pre-commit's `identify` library uses **`c++`**, not `cpp`, for C++ files:
...
| SAFE801 | tainted_sink | Shares the C tracker's node handling via a `CppTaintTracker` subclass that adds the method-receiver step C omits (`req->param("q")` stays tainted). Reference parameters (`const std::string& s`) seed taint, and a reference declaration (`const std::string& r = tainted;`) propagates it. Explicitly instantiated template calls (`execute<int>(x)`, `db.query<Row>(x)`) resolve to their bare name, so a sink configured as `execute` / `query` matches. Sinks / sources / sanitizers via `sinks_cpp` / `sources_cpp` / `sanitizers_cpp` (mirror C). Disabled by default. **Property-typed sanitisers (2.13.0):** the flat `sanitizers_cpp` list is universal - a name there clears *every* sink. To recognise a context-specific encoder precisely, declare what it establishes in `sanitizer_properties_cpp` (name -> list of properties) and what each sink requires in `sink_properties_cpp` (sink -> one property); it then clears only the sinks whose required property it establishes. Keep such a name OUT of the flat list, which is checked first and would otherwise clear everything. Opt-in - both tables ship empty, so existing config is unaffected. |
