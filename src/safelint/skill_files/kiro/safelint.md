...
2. **Per-file breakdown** (skip files with zero violations). For each file, list violations one per line:
   ...
   Pad codes / line numbers / messages so columns line up. Don't emit ANSI colour, Kiro renders the text directly.
