...
This file is a sibling to the Anthropic Claude Code skill (`SKILL.md`), the Cursor project rule (`cursor/safelint.mdc`), and the GitHub Copilot instructions (`copilot/copilot-instructions.md`) that ship in the same package — every supported AI client reads the same workflow so behaviour stays consistent. Gemini CLI auto-discovers `GEMINI.md` at the repo root (project scope); for user-scope global instructions the file lives at `~/.gemini/GEMINI.md` and must be referenced via Gemini CLI configuration. Language-specific addendums and the schema doc live in the bundled package and can be located via `safelint skill path` if you need them.
