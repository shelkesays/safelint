...
DEFAULTS: dict[str, Any] = {
    ...
    "rules": {
        ...
        "tainted_sink": {
            ...
            # Property-typed sanitiser contract (opt-in, empty by default).
            # ``sanitizer_properties`` maps a sanitiser to the safety properties it
            # establishes; ``sink_properties`` maps a sink to the one property it
            # requires. A property-typed sanitiser clears a sink only when it
            # establishes that sink's required property, so an HTML escaper does not
            # clear a SQL sink. The flat ``sanitizers`` list stays universal (clears
            # every sink) for backward compatibility; only that flat list clears a
            # sink with no declared property. Python keys are bare; other languages
            # use the ``_<lang>`` suffix. Both ship EMPTY - the mechanism is entirely
            # opt-in, so the taint labelling adds no cost until a project configures
            # it (see docs/configuration/rules.md for the Pydantic example, which is
            # a recommended config rather than a shipped default so it never adds
            # inert per-property work).
