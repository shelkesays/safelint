...
def test_file_summary_line_warning_only() -> None:
    ...
    line = _file_summary_line("path/file.py", [_v("warning")])
...
def test_file_summary_line_warnings_plural() -> None:
    ...
    line = _file_summary_line("path/file.py", [_v("warning"), _v("warning")])
...
def test_make_summary_no_violations() -> None:
    ...
    assert found == "All checks passed."
    assert "No fixes available" in fixes
...
def test_make_summary_suppressed_note() -> None:
    ...
    assert "3 suppressed" in found
    assert "3 suppressed" in fixes
