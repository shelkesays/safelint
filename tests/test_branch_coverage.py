...
from safelint.languages._node_utils import call_has_arguments as _call_has_arguments
...
def test_call_has_arguments_all_branches() -> None:
    """``call_has_arguments``: real args -> True; empty / comment-only / no field -> False.
    ...
    assert _call_has_arguments(_first_call("f(1)\n")) is True
    assert _call_has_arguments(_first_call("f()\n")) is False
    assert _call_has_arguments(_first_call("f(  # a comment\n)\n")) is False
    ...
    assert _call_has_arguments(js_call) is False

    class _NoArgsNode:
        def child_by_field_name(self, _name: str) -> None:
            return None  # a node with no ``arguments`` field (e.g. paren-less ``new Foo``)

    assert _call_has_arguments(cast("tree_sitter.Node", _NoArgsNode())) is False
