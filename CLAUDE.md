...
- **`analysis/`**: shared analyses used by multiple rules. Four intra-procedural taint trackers, one per language family: `dataflow.py` (Python `TaintTracker`), `dataflow_javascript.py`, `dataflow_java.py`, `dataflow_rust.py`, consumed by the dataflow rules (`tainted_sink`, `return_value_ignored`, `null_dereference`). All tree-walking in them is iterative (worklists), never recursive - SAFE105 `no_recursion` polices safelint's own source.
