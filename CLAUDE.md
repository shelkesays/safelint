...
- **`analysis/`**: shared analyses used by multiple rules. Seven intra-procedural taint trackers, one per language family: `dataflow.py` (Python `TaintTracker`), `dataflow_javascript.py`, `dataflow_java.py`, `dataflow_rust.py`, `dataflow_go.py`, `dataflow_php.py`, `dataflow_c.py` (C and C++, the latter via the `CppTaintTracker` subclass), consumed by the dataflow rules (`tainted_sink`, `return_value_ignored`, `null_dereference`; Go skips `null_dereference`). All tree-walking in them is iterative (worklists), never recursive - SAFE105 `no_recursion` polices safelint's own source.
...
GitHub Actions (`ci.yml`) runs the same lint/format/type checks (against `src/`, `tests/`, and `scripts/`) on a single Python version, then the pytest suite across Python 3.11 / 3.12 / 3.13 / 3.14.

**Releases are automated - the version bump is the trigger.** `publish.yml` is **branch-triggered** (push to `main` / `development`), not tag-triggered. On each push a gate reads `project.version` from `pyproject.toml`; when it is a **new, untagged** version on its **expected branch** (a pre-release `X.Y.ZrcN` from `development`, a final `X.Y.Z` from `main`), the workflow tags it, builds, publishes to PyPI (Trusted Publishing / OIDC, no tokens), and creates the GitHub release. Every other push (version unchanged, or wrong branch for the version type) hits the gate and skips. A companion `sync-development.yml` resets `development` to `main` after each push to `main` (reset-if-safe: only when `development` has nothing `main` lacks, else it opens a tracking issue and fails), so the two branches never diverge and the next release branch always rebases cleanly.

Release mechanics: the package version is **static** in `pyproject.toml` (no `setuptools_scm`). The bump *is* the deliberate committed edit that authorises a release. RC releases publish to the `pypi-rc` environment (hands-off) as GitHub **pre-releases**; final releases publish to the reviewer-gated `pypi` environment (a human approves the production upload) and, on that final, the workflow **auto-dates the CHANGELOG** (`## [Unreleased]` → `## [X.Y.Z] - <date>`, re-adds an empty `[Unreleased]`, updates the compare footers) and commits it to `main` before tagging. Release notes come from the CHANGELOG section (`scripts/changelog_section.py`). The whole `pull → tag → push → release → date` sequence is gone.

**Release workflow (branch flow - the version bump is still yours, everything after the merge is automatic).** Development happens on a `feature/*` branch which **PRs into `development`, NOT directly into `main`**:

1. `feature/* → PR → development`: the PR **includes** the version bump to the next **RC** (`project.version = "X.Y.ZrcN"`, e.g. `2.9.0rc1`; iterate `rc2`, ... on further rounds) and the CHANGELOG entries under `## [Unreleased]` (do **not** date the heading - the workflow does that at the final). Merging it auto-releases the RC (tag `vX.Y.ZrcN` + PyPI pre-release + GitHub pre-release).
2. `development → PR → main`: flip `project.version` to the **production** `"X.Y.Z"`. Merging it auto-releases the final: auto-dated CHANGELOG, tag `vX.Y.Z`, PyPI (behind the `pypi` reviewer gate), GitHub release, and `development` re-synced to `main`.

What stays **manual**: choosing the version *number* (additive = next `X.Y.0`; internal tooling / bugfix = next `X.Y.Z` patch; never MAJOR - the owner controls timing), **writing** the CHANGELOG entries, and reviewing/merging the PRs. What is now **automated**: tagging, PyPI publish, GitHub release, CHANGELOG dating, and the `development`/`main` sync. The full design + one-time setup (RELEASE_PAT, `pypi`/`pypi-rc` environments, PyPI trust) is in `plan/release-automation.md` and the release-process doc.
