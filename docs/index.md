...
|----------|------------|-------|
| **Python** | `.py`, `.pyw` | Default language; the rule set was originally designed for Python and ports unchanged. Web-framework *defaults* are switchable via the `[tool.safelint.python] framework = "..."` preset (`django` / `flask` / `fastapi`) plus an orthogonal `pydantic = true`, which selectively enable the shared SAFE905-909 rules (Django enables all five; Flask enables `debug_mode_enabled` + `unvalidated_request_input` + `hardcoded_secret`; FastAPI enables `debug_mode_enabled` + `unvalidated_request_input`; `pydantic = true` enables `mass_assignment`). See [Python](languages/python.md). |
...
| Python / PHP framework presets only | 5 | `debug_mode_enabled` (SAFE905), `mass_assignment` (SAFE906), `unvalidated_request_input` (SAFE907), `csrf_protection_disabled` (SAFE908), `hardcoded_secret` (SAFE909); enabled by the Python / PHP framework presets (Django / Laravel: all five; Flask: SAFE905 + SAFE907 + SAFE909; FastAPI: SAFE905 + SAFE907; `pydantic = true`: SAFE906) |
