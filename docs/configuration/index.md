...
A locally-customised install will surface as *differs from bundled*; the diagnostic message explicitly mentions that case so customisers can ignore it. See [AI client integrations](../ai-clients/index.md) "Updating after a safelint upgrade" for the full workflow.
...
## File-level suppression (`# safelint: ignore`)
...
### Choosing between the four suppression mechanisms

You now have four ways to silence a rule, ranging from narrowest to widest:
...
The lower-scope forms compose with the higher-scope forms — toml-level ignores still apply on top of file-level directives, and inline `# nosafe` always wins for the specific line it's on.
