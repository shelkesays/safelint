...
| **Git-modified mode** (default `safelint check src/`), user modified files but every one was dropped by the supported-extensions filter | the silent-pass guard in `_run_check` / `_finish_unlinted` | `safelint: error: no files linted - every git-modified or discovered source file has a grammar that isn't installed - install with: pip install 'safelint[<lang>]'` |
