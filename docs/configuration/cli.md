...
|---|---|---|
| `--all-files` | off | Scan every supported source file under the target (`.py`, `.pyw`, `.js`, `.mjs`, `.cjs`, `.ts`, `.tsx`, `.as`, `.java`, `.rs`, `.go`, `.php`). Default (without this flag) is to check only git-modified files. |
...
| Flag | Default | What it does |
|---|---|---|
| `--language <LANG>` | (no filter) | Restrict to one language: `python`, `javascript`, `typescript`, `java`, `rust`, `go`, `php`. Filters by the rule's `language` tuple, so cross-language rules show under any of their languages. |
