...
- **Assignment to a sink-named property (JavaScript / TypeScript only)** - `element.innerHTML = tainted` is reported, as are the `element["innerHTML"] = tainted` subscript spelling and `element.innerHTML += tainted`. Writing the property *is* the injection, so the target is checked against the same `sinks_*` list, with the same sanitizer and property-typed semantics as a call. Any configured name works this way, so `outerHTML` / `srcdoc` can be opted in via `sinks_javascript`. **This is JS/TS-only**: in every other language a sink is recognised only when it is *called*, so configuring a property-style sink name there has no effect on assignments to it. *Added in 2.14.0.*
