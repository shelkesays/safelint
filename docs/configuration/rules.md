...
- **Python** (`.py`, `.pyw`). Per-framework *defaults* (taint sinks, nullable methods, and which of the shared `SAFE905-909` framework rules are active) are switchable via the [`[tool.safelint.python] framework = "..."`](../languages/python.md#framework-presets) preset (`vanilla` / `django` / `flask` / `fastapi`), with an orthogonal `pydantic = true` axis.
...
| **Python / PHP framework presets only** | 5 | SAFE905 (`debug_mode_enabled`), SAFE906 (`mass_assignment`), SAFE907 (`unvalidated_request_input`), SAFE908 (`csrf_protection_disabled`), SAFE909 (`hardcoded_secret`); all default-disabled, enabled by the Python / PHP framework presets (Django / Laravel enable all five; Flask enables SAFE905 + SAFE907 + SAFE909; FastAPI enables SAFE905 + SAFE907; `pydantic = true` enables SAFE906). The non-Java analogue of the Spring `SAFE9xx` rules. |
...
Matching is by call name only (receiver stripped) - the same allowlist approach every other language's SAFE601 detector and `tainted_sink.sanitizers` use, and the reason a *bare* project helper (`verify_invariant(x)`) counts. The tradeoff: a production call whose name coincides with a configured entry (a helper literally named `fail` / `raises`) is also credited, so SAFE601 may under-report an assertion-less function. This is a false-negative on an opt-in advisory rule; enable `test_functions_only` (below) to confine the rule to test functions, where these names are unambiguously assertions.
...
- **Go** (comments): golangci-lint's bare `//nolint` (all linters) and Staticcheck's bare `//lint:ignore` (no check list). Both are machine-readable directives recognised only in the tight form with no space after `//` (`//nolint`, not `// nolint`), so a prose `// nolint here` comment is left alone. Scoped (`//nolint:errcheck`, `//lint:ignore SA1000 reason`) is clean.
...
- **Assignment to a sink-named property** - `element.innerHTML = tainted` is reported, as are the `element["innerHTML"] = tainted` subscript spelling and `element.innerHTML += tainted`. Writing the property *is* the injection, so the target is matched with the same sanitizer and property-typed semantics as a call. Write-sinks live in their own **`assignment_sinks`** list (`assignment_sinks_<lang>`, bare for Python), NOT the flat `sinks` list: outside JavaScript those hold function names, so reusing them would report every field called `query` / `args` / `load` as an injection. Only `innerHTML` ships as a default (JavaScript, inherited by TypeScript); add `outerHTML` / `srcdoc` or a project-specific setter yourself. Supported in all nine languages on their native member shapes. A **computed** subscript key (`results[key] = tainted`) is not matched - its spelling names no property - while a quoted one (`o["danger"]`) is. *Added in 2.14.0.*
...
| `assignment_sinks` | JavaScript: `innerHTML` | Sinks that are **written to** rather than called (`el.innerHTML = tainted`). Separate from `sinks` because those are function names. *Added in 2.14.0.* |
| `sanitizer_properties` | `{}` | Table of sanitiser name -> the safety properties it establishes (e.g. `escape = ["html_escaped"]`). Opt-in; see [Property-typed sanitisers](#property-typed-sanitisers-sanitizer_properties-sink_properties). *Added in 2.13.0.* |
| `sink_properties` | `{}` | Table of sink name -> the single safety property it requires (e.g. `RawSQL = "sql_escaped"`). Opt-in; see [Property-typed sanitisers](#property-typed-sanitisers-sanitizer_properties-sink_properties). *Added in 2.13.0.* |
...
A property-typed sanitiser's clear does **not** survive an intervening unknown call, including a benign-looking wrapper: `html_sink(str(escape(u)))` and `html_sink(escape(u).strip())` both report, because an unknown call may equally well decode its input. The flat `sanitizers` list does not behave this way - it short-circuits before descending - so the precise form is deliberately the stricter one. Wrap-then-sanitise (`escape(str(u))`) avoids it.
...
**What it flags:** CSRF protection explicitly turned off in code. Python (Django) + PHP (Laravel). New in v2.11.0.

CSRF protection is on by default in these frameworks; disabling it opens a hole an attacker can drive state-changing requests through. Detected patterns:
...
| Option | Default | Description |
|---|---|---|
| `enabled` | `false` (vanilla) / `true` (Django + Laravel presets) | Toggle the rule |
| `severity` | `"error"` | `"error"` or `"warning"` |
...
```python
@csrf_exempt
def webhook(request):        # SAFE908: CSRF protection disabled
    ...
```
...
class VerifyCsrfToken extends Middleware {
    protected $except = ['stripe/*'];   // SAFE908: routes exempted from CSRF
}
```

The Django / Laravel presets enable this rule automatically; enable it directly with either TOML layout:

```toml
# pyproject.toml
[tool.safelint.rules.csrf_protection_disabled]
enabled = true
severity = "error"
```
...
```toml
# safelint.toml
[rules.csrf_protection_disabled]
enabled = true
severity = "error"
```

### SAFE909: `hardcoded_secret`

**What it flags:** A secret key assigned a string literal in code. Python (Django / Flask) + PHP (Laravel). New in v2.11.0.

A committed secret is a credential leak the moment it lands in version control. Detected patterns:
...
| Option | Default | Description |
|---|---|---|
| `enabled` | `false` (vanilla) / `true` (Django + Flask + Laravel presets) | Toggle the rule |
| `severity` | `"error"` | `"error"` or `"warning"` |

**Bad:**

```python
SECRET_KEY = "django-insecure-9a3f..."   # SAFE909: hardcoded secret
```
...
```python
import os
SECRET_KEY = os.environ["SECRET_KEY"]    # loaded from the environment - clean
```

The Django / Flask / Laravel presets enable this rule automatically; enable it directly with either TOML layout:

```toml
# pyproject.toml
[tool.safelint.rules.hardcoded_secret]
enabled = true
severity = "error"
```
