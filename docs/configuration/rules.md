...
- **Assignment to a sink-named property** - `element.innerHTML = tainted` is reported, as are the `element["innerHTML"] = tainted` subscript spelling and `element.innerHTML += tainted`. Writing the property *is* the injection, so the target is checked against the same `sinks_*` list, with the same sanitizer and property-typed semantics as a call. Supported in **all nine languages** on their native member shapes (`obj.attr` / `obj["attr"]` in Python, `field_access` in Java, `obj.field` in Rust and Go, `$obj->prop` in PHP, `obj.field` / `obj->field` in C and C++). Any configured name works this way, so `outerHTML` / `srcdoc` can be opted in via `sinks_javascript`, and a project-specific setter via its own `sinks_<lang>`. *Added in 2.14.0.*
