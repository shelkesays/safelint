...
5. **Rules** import per-language node-type constants from the matching `languages/<lang>.py` module (e.g. `from safelint.languages.python import FUNCTION_DEF`); as of v2.8.2 every per-language table row and helper references its own language's constant, never a raw Tree-sitter string literal. So porting a rule to a new language means adding that language's constants to its module and referencing them in the rule's per-language table - never hardcoding `"if_statement"`-style strings. Per-rule porting is what each new language triggers; the engine plumbing for it is already in place.
...
* The [Rules reference](../configuration/rules.md): add the language to the "Currently supported" list and the "Rule scope (current languages)" table, and **remove it from "Planned"**. (The generated at-a-glance rules table is `Code | Name | Default severity | Enabled by default` only - it carries no language column, so it needs no manual edit *and* does not convey language coverage; that lives in the scope table and the per-language pages.) Add a `docs/languages/<lang>.md` page and its `mkdocs.yml` nav entry.
