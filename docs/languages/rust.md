...
| [SAFE105](../configuration/rules.md#safe105-no_recursion) | `no_recursion` | Flags a `function_item` that calls itself directly, bare (`fact(n-1)`) or `self`-qualified (`self.walk(...)`). `other.walk(...)` does not fire. Direct self-recursion only (mutual recursion is out of scope). Enabled by default at warning severity. |
...
| [SAFE603](../configuration/rules.md#safe603-blanket_suppression) | `blanket_suppression` | Flags blanket allows (Holzmann rule 10): `#[allow(clippy::all)]`, `#[allow(warnings)]`, and their inner `#![...]` forms. Scoped allows (`#[allow(dead_code)]`, `#[allow(clippy::too_many_arguments)]`) are clean. Disabled by default. |
...
| [SAFE801](../configuration/rules.md#safe801-tainted_sink) | `tainted_sink` | Method parameters seeded as tainted on function entry. Default vanilla sinks (`sinks_rust`): `arg` / `args` (`cmd.arg(tainted)` / `cmd.args(tainted)`), `query` / `query_as` / `query_scalar` / `execute` / `execute_batch` (raw-SQL across sqlx / diesel / rusqlite / postgres), `open` (filesystem paths). **`Command` / `new` / `Library` are NOT defaults** even though `Command::new(tainted)` is a real hazard: `call_name` resolves `Command::new(...)` to the bareword `new`, so the type names `Command` / `Library` can never match, and `new` itself would fire on every `String::new` / `Vec::new` / `Box::new` constructor taking a tainted argument. `Command::new(tainted)` is instead caught via `.arg(tainted)` / `.args(tainted)`, the idiomatic shape (the program name is almost always a literal); opt `new` into `sinks_rust` yourself if you accept the false-positive cost. A tainted **executable name** with no argument method - `Command::new(user_input).status()` - is therefore NOT reported by default; `.arg` / `.args` cover tainted arguments, not the program being run. Opt `new` into `sinks_rust` if you need that flow, accepting the constructor false positives. Vanilla sources (`sources_rust`): `var` (`std::env::var`) and `read_to_string` (`std::fs::read_to_string`) - call names whose RETURN carries user data. Out-parameter / count-returning calls (`read_line`, `recv`, `recv_from`, `lock`) and bareword-colliding builders (`args` collides with `Command::args`) are intentionally NOT in defaults to keep noise down. Vanilla sanitizers (`sanitizers_rust`): narrow generic-wrapper set (`validate` / `sanitize` / `escape` / `quote`) - all universal (the flat `sanitizers_rust` list clears every sink); context-specific encoders are deliberately NOT in that flat list. To recognise a context-specific encoder precisely (clearing only the sinks it protects), use the [property-typed sanitiser contract](../configuration/rules.md#property-typed-sanitisers-sanitizer_properties-sink_properties) (`sanitizer_properties_rust` + `sink_properties_rust`, 2.13.0) rather than the universal `sanitizers_rust` list. **Macro-based sinks** (`sqlx::query!`) are a documented limitation - they parse as `macro_invocation`, not `call_expression`, and the macro body is a token tree we don't currently decode. Disabled by default. |
...
| [SAFE307](../configuration/rules.md#safe307-interior_mutable_static) | `interior_mutable_static` | **Holzmann rule 6 (smallest scope).** Fires on a `static` whose type provides safe interior mutability (`Mutex` / `RwLock` / `OnceLock` / `OnceCell` / `Lazy` / `LazyLock` / `Atomic*` / `RefCell` / `Cell`) and on `lazy_static! { ... }` blocks. This is the safe-code global-mutable-state that SAFE602's `unsafe` gate never sees. `const` and `static mut` are not flagged. Word-boundary matching keeps `Lazy` from matching a user `LazyLoader` type. Configurable via `interior_mutable_types_rust`. Disabled by default. |
...
| [SAFE301](../configuration/rules.md#safe301-global_state) | `global_state` | Rust has no `global` keyword. `static mut FOO` is the closest analogue but requires `unsafe { }` to access, so it's already audit-gated via **SAFE602 `undocumented_unsafe`**. Safe interior-mutable statics are covered separately by **SAFE307 `interior_mutable_static`**. |
| [SAFE302](../configuration/rules.md#safe302-global_mutation) | `global_mutation` | Same as SAFE301 - `static mut` writes are unsafe-gated (SAFE602), and safe interior-mutable statics (`Mutex` / `OnceLock` / `lazy_static!`) are covered by **SAFE307 `interior_mutable_static`**. |
| [SAFE309](../configuration/rules.md#safe309-dynamic_code_execution) | `dynamic_code_execution` | Rust's rule-8 analogue is the macro system, whose bodies parse as opaque token trees safelint cannot decode (the same limitation noted for SAFE801's `sqlx::query!`). `panic`-family macros already have **SAFE204**. A general macro-as-codegen rule is not offered. |
...
All 11 Rust-only rules ship disabled by default. Opt-in via TOML:
