...
| [SAFE601](../configuration/rules.md#safe601-missing_assertions) | `missing_assertions` | Functions with fewer than `min_assertions` assertions - the `assert` keyword plus `assertion_calls` method calls (`self.assertEqual(...)` / `pytest.raises(...)`; default 1, set 2 for Holzmann rule 5's density). Disabled by default. |
...
- **`[tool.safelint.rules.tainted_sink]`**, `sinks`, `sanitizers`, `sources` default to Python's threat surface (`eval`, `exec`, `subprocess`, …). The `_javascript`-suffixed equivalents are independent lists. Python uses the **bare** key names for the opt-in [property-typed sanitiser contract](../configuration/rules.md#property-typed-sanitisers-sanitizer_properties-sink_properties) too - `sanitizer_properties` / `sink_properties` (2.13.0), not `_python`-suffixed. Use them to recognise a context-specific escaper precisely, so it clears an HTML sink without also clearing `RawSQL`. **The flat list wins, so you must remove the name from it first:** `escape` ships in the default `sanitizers`, and the flat list is consulted before the property tables, so declaring properties for it changes nothing until you also set `sanitizers` without it. safelint warns when a name sits in both places. **Configure the bareword, not the qualified path** - names are matched after `call_name` reduces the callee, so `html.escape(x)` matches an `escape` entry and an `html.escape` entry matches nothing.
