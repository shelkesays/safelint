...
Java is in scope for the cross-language rules plus SAFE302 `global_mutation` (added in 2.4.0, non-final-static-field check) and the 4 Spring Boot framework-specific rules (`SAFE9xx`, enabled by the `spring-boot` preset). Also new in 2.4.0: SAFE105 `no_recursion` (enabled by default), SAFE309 `dynamic_code_execution`, and SAFE603 `blanket_suppression` (both disabled by default). 3 rules are deliberately skipped, see the next section.
...
| [SAFE105](../configuration/rules.md#safe105-no_recursion) | `no_recursion` | Flags a `method_declaration` that calls itself directly, bare (`fact(n-1)`) or `this`-qualified (`this.fact(n-1)`). `other.fact(...)` does not fire. Direct self-recursion only (mutual recursion is out of scope). Enabled by default at warning severity. |
...
| [SAFE302](../configuration/rules.md#safe302-global_mutation) | `global_mutation` | *(added in 2.4.0)* Fires on non-final `static` field declarations, Java's shared-mutable-state shape (Holzmann rule 6). **Declaration-site** detection: a non-final static field IS the violation regardless of where it is written. `static final` is clean (even when the referent is interiorly mutable, a documented exclusion needing type resolution). Instance fields, locals, and interface fields (implicitly `public static final`) never fire. |
...
| [SAFE309](../configuration/rules.md#safe309-dynamic_code_execution) | `dynamic_code_execution` | Structural detection of reflection (Holzmann rule 8): `Class.forName` / `Method.invoke` / `eval` (JSR-223) / `defineClass` / `loadClass`, matched by method name. Complements SAFE801 (taint-gated). Disabled by default. |
...
| [SAFE603](../configuration/rules.md#safe603-blanket_suppression) | `blanket_suppression` | Flags `@SuppressWarnings("all")` and `@SuppressWarnings({..., "all"})` (Holzmann rule 10). Scoped suppressions like `@SuppressWarnings("unchecked")` are clean. Disabled by default. |
...
| [SAFE801](../configuration/rules.md#safe801-tainted_sink) | `tainted_sink` | Method parameters are seeded as tainted on function entry (every `formal_parameter` / `spread_parameter` name; `receiver_parameter` `Foo this` is excluded). Default vanilla sinks: `exec` / `getRuntime` / `ProcessBuilder` (command execution), `loadLibrary` / `load` (native libraries), `forName` / `invoke` / `newInstance` (reflection), `eval` (JSR-223), `executeQuery` / `execute` / `executeUpdate` / `executeLargeUpdate` (JDBC), `openConnection` / `openStream` (URL). Default vanilla sources: `getenv` / `getProperty` (environment), `readLine` / `nextLine` / `next` / `nextInt` (console / Scanner), and the Servlet API (`getParameter` / `getParameterValues` / `getHeader` / `getHeaders` / `getQueryString` / `getCookies` / `getPathInfo` / `getRequestURI` / `getRemoteUser`). The receiver-payload SSRF sinks `openConnection` / `openStream` ship as `receiver_sinks_java` defaults (they fire on a tainted `URL` receiver even with clean arguments; see the [`receiver_sinks`](../configuration/rules.md#safe801-tainted_sink) note in rules.md). Default vanilla sanitizers are limited to generic validators / wrappers: `sanitize`, `validate`, `quote`, `escape` - all universal (the flat `sanitizers_java` list clears **every** sink). Context-specific encoders are intentionally **not** in that flat list because a URL/HTML/XML encoder doesn't make input safe for SQL / shell / reflection. Specifically excluded from the flat list: URL encoders (`URLEncoder.encode`, matched as `encode`), Apache Commons HTML/XML escapers (`escapeHtml*` / `escapeXml` / `escapeJava` / `escapeJson`), Spring `htmlEscape`, and OWASP Java Encoder methods (`forHtml` / `forJavaScript` / `forCssString` / `forUri` / `forXml`). To recognise a context-specific encoder **precisely** - clearing only the sinks it actually protects - declare it with the [property-typed sanitiser contract](../configuration/rules.md#property-typed-sanitisers-sanitizer_properties-sink_properties) (`sanitizer_properties_java` + `sink_properties_java`, 2.13.0) rather than dropping it in the universal `sanitizers_java` list. The `spring-boot` preset adds `JdbcTemplate.query` / `queryForObject` / `queryForList` / `queryForMap` / `queryForRowSet` / `batchUpdate` and `RestTemplate.getForObject` / `getForEntity` / `postForObject` / `postForEntity` / `postForLocation` / `patchForObject`. Bare `put` / `delete` / `update` / `exchange` are deliberately omitted from defaults because they collide heavily with HashMap / File / project-local helpers; add them explicitly via `[tool.safelint.rules.tainted_sink] sinks_java` if you specifically need RestTemplate.put / .delete / .exchange SSRF coverage or jdbcTemplate.update SQLi coverage. Disabled by default. |
...
- `[tool.safelint.rules.tainted_sink]`, `sinks_java`, `sanitizers_java`, `sources_java`, `receiver_sinks_java`, `sanitizer_properties_java`, `sink_properties_java`
