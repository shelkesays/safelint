...
The `--pre` flag plus the `==2.1.0rc1` pin are both needed while the RC is the current release: pip defaults to stable, and the latest stable (v2.0.0) does not have a `[java]` extra. Drop both once v2.1.0 GA is published. If your Java project doesn't already have a Python tool chain, `pipx install --pre 'safelint[java]==2.1.0rc1'` isolates the install. Maven / Gradle plugins are NOT required; safelint is a standalone CLI that reads source files directly.
...
| [SAFE304](../configuration/rules.md#safe304-side_effects) | `side_effects` | Fires on any method whose name doesn't *signal* I/O. Java default `io_functions_java` set is a deliberately narrower subset of SAFE303's: `println` / `print` / `printf`, `readLine` / `nextLine`, file IO via `new` (`FileInputStream` / `FileOutputStream` / `FileReader` / `FileWriter` / `Scanner`), and `java.nio.file.Files` static methods (`readAllBytes` / `readAllLines` / `writeString` / `write`). SAFE303-only entries (`BufferedReader` / `BufferedWriter` / `PrintWriter` / `Socket` / `newHttpClient` / `send` / `sendAsync` / `printf`) are NOT in SAFE304's list because they're often legitimately invoked from non-pure helpers where flagging would be noisy. **Spring users:** the `spring-boot` preset does NOT exempt `@Bean` factory methods - if you have noisy hits on factory methods that legitimately create side-effectful resources, suppress with `// nosafe: SAFE304` until a future `skip_functions_annotated_with` knob lands. |
